...
const contactInfo = [
  {
    ...
    color: "from-blue-500/12 to-blue-500/4", iconBg: "bg-blue-500/10", iconColor: "text-blue-600",
  },
  {
    icon: Phone, label: "Phone Numbers",
    value: "7676272167 • 7975344252 • 8618181383 • 7892508243",
    link: "tel:7676272167",
    color: "from-emerald-500/12 to-emerald-500/4", iconBg: "bg-emerald-500/10", iconColor: "text-emerald-600",
  },
  {
    icon: Mail, label: "Email",
    value: "principal.hoysaladegreecollege@gmail.com",
    link: "mailto:principal.hoysaladegreecollege@gmail.com",
    color: "from-primary/12 to-primary/4", iconBg: "bg-primary/10", iconColor: "text-primary",
  },
  {
    icon: Clock, label: "Office Hours",
    value: "Mon – Sat: 9:00 AM – 5:00 PM",
    link: undefined,
    color: "from-secondary/15 to-secondary/4", iconBg: "bg-secondary/15", iconColor: "text-secondary-foreground",
  },
];
...
export default function Contact() {
  ...
  const inputClass = "w-full border border-border rounded-xl px-4 py-3 font-body text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary/40 transition-all duration-300 placeholder:text-muted-foreground/40 hover:border-primary/25";
  ...
  return (
    <div className="page-enter">
      ...
      <section className="py-16 sm:py-24 bg-background relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-56 h-56 bg-primary/3 rounded-full blur-3xl pointer-events-none" />

        <div className="container max-w-5xl px-4 relative">
          <ScrollReveal><SectionHeading title="Get in Touch" subtitle="Reach out for admissions, queries, or campus visits." /></ScrollReveal>
          ...
          <div className="grid md:grid-cols-2 gap-10 sm:gap-12 mt-10">
            ...
            <ScrollReveal>
              <div className="space-y-3">
                {contactInfo.map((item, i) => (
                  <div key={item.label}
                    className={`flex gap-4 items-start p-4 rounded-2xl bg-gradient-to-br ${item.color} border border-border/40 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group`}
                    style={{ animationDelay: `${i * 80}ms` }}>
                    <div className={`w-11 h-11 rounded-xl ${item.iconBg} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                    ...
                    <div className="min-w-0">
                      ...
                      {item.link ? (
                        <a href={item.link} target={item.external ? "_blank" : undefined} rel={item.external ? "noopener noreferrer" : undefined}
                          className="font-body text-sm text-muted-foreground hover:text-primary transition-colors duration-200 break-words">{item.value}</a>
                      ) : (
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {["7676272167", "7975344252", "8618181383", "7892508243"].map((num) => (
                    <a key={num} href={`tel:${num}`}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-card border border-border font-body text-xs text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary hover:scale-105 transition-all duration-300 shadow-sm">
            ...
            <ScrollReveal delay={200}>
              <div className="premium-card p-6 sm:p-8 relative overflow-hidden">
                <div className="absolute -top-12 -right-12 w-40 h-40 bg-secondary/6 rounded-full blur-2xl pointer-events-none" />

                {submitted ? (
                  <div className="flex flex-col items-center justify-center h-72 animate-scale-bounce text-center">
                    <div className="relative w-20 h-20 rounded-2xl bg-emerald-100 flex items-center justify-center mb-5 shadow-lg">
                    ...
                    <p className="font-body text-sm text-muted-foreground">We'll get back to you soon. Thank you for reaching out!</p>
                    <div className="mt-4 w-32 h-1 bg-gradient-to-r from-emerald-300 via-emerald-500 to-emerald-300 rounded-full animate-gradient" />
                  </div>
                ) : (
                  <>
                    ...
                    <form className="space-y-4 relative z-10" onSubmit={handleSubmit}>
                      ...
                      <div>
                        ...
                        <textarea rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })}
                          className={`${inputClass} resize-none`} placeholder="Write your message here..." />
                      </div>
                      <button type="submit" disabled={submitting}
                        className="relative w-full group overflow-hidden px-6 py-3 rounded-xl font-body text-sm font-bold text-primary-foreground transition-all duration-300 hover:scale-[1.02] disabled:opacity-60 shadow-lg"
