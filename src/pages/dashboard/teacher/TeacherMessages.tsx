...
import { MessageCircle, ArrowLeft, User, Send, Inbox, CheckCheck } from "lucide-react";
...
export default function TeacherMessages() {
  ...
  const { data: threadReplies = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from("direct_messages").select("*").eq("parent_message_id", activeThread).order("created_at");
      return data || [];
    },
  ...
  const { data: profileMap = {} } = useQuery({
    queryKey: ["teacher-msg-profiles", conversations, threadReplies],
    queryFn: async () => {
  ...
  const sendReply = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("direct_messages").insert({
        ...
        subject: parentMsg.subject, message: replyText.trim(), parent_message_id: activeThread,
      });
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="bg-card border border-border/60 rounded-2xl p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <Link to="/dashboard/teacher" className="p-2 rounded-xl hover:bg-muted transition-colors shrink-0">
          ...
          <div>
            ...
            <p className="font-body text-xs text-muted-foreground mt-0.5">
              {unreadCount > 0 ? `${unreadCount} unread message${unreadCount > 1 ? "s" : ""}` : "View and reply to student questions"}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="p-4 border-b border-border flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body text-[11px] text-muted-foreground">
                from {(profileMap as any)[activeMsg.sender_id === user?.id ? activeMsg.receiver_id : activeMsg.sender_id] || "Student"}
              </p>
            </div>
          </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl overflow-hidden">
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="font-body text-xs text-muted-foreground/60 mt-1">Students can send you questions from their dashboard</p>
            </div>
          ) : (
            <div className="divide-y divide-border/40">
              {conversations.map((m: any) => {
                ...
                return (
                  <button key={m.id} onClick={() => { setActiveThread(m.id); markRead(m.id); }}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className={`font-body text-[12px] font-semibold truncate ${isUnread ? "text-foreground" : "text-foreground/80"}`}>
                          {(profileMap as any)[otherId] || "Student"}
                        </p>
