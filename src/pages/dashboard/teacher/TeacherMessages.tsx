...
export default function TeacherMessages() {
  ...
  const { data: studentContacts = [] } = useQuery({
    queryKey: ["teacher-msg-students"],
    queryFn: async () => {
      const { data: studentRows } = await supabase.from("students").select("id, user_id, roll_number, course_id").eq("is_active", true);
  ...
  const { data: teacherContacts = [] } = useQuery({
    queryKey: ["teacher-msg-teachers", user?.id],
    queryFn: async () => {
      if (!user) return [];
      const { data: teacherRows } = await supabase.from("teachers").select("id, user_id, employee_id, subjects").eq("is_active", true).neq("user_id", user.id);
  ...
  const { data: conversations = [], refetch: refetchConversations } = useQuery({
    queryKey: ["teacher-conversations", user?.id],
    queryFn: async () => {
      if (!user) return [];
  ...
  const { data: threadMessages = [], refetch: refetchThread } = useQuery({
    queryKey: ["teacher-thread", user?.id, selectedContactId],
    queryFn: async () => {
