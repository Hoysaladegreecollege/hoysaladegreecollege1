...
export default function StudentProfile() {
  ...
  const handleRegisterPasskey = async () => {
    ...
    try {
      ...
      const { data: optionsData, error: optionsError } = await supabase.functions.invoke("passkey-register", {
        body: { action: "get-options" },
      });

      if (optionsError || optionsData?.error) {
        toast.error(optionsError?.message || optionsData?.error || "Failed to load passkey options");
        return;
      }
      ...
      if (registerError || registerData?.error) {
        toast.error(registerError?.message || registerData?.error || "Passkey registration failed");
        return;
      }
      ...
      <div className="relative overflow-hidden bg-card border border-border/40 rounded-3xl p-6 sm:p-8">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-body text-xs text-muted-foreground mt-0.5">View your details and manage passkeys</p>
          </div>
        </div>
      </div>
