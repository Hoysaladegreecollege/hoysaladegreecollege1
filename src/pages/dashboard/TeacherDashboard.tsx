...
import { Users, Clock, BarChart3, Upload, Bell, Megaphone, TrendingUp, Calendar } from "lucide-react";
...
export default function TeacherDashboard() {
  ...
  const stats = [
    ...
    { label: "Materials", value: String(counts?.materials ?? "—"), icon: Upload, gradient: "from-secondary/8 to-secondary/3" },
    { label: "Active Notices", value: String(counts?.notices ?? "—"), icon: Bell, gradient: "from-primary/8 to-primary/3" },
  ];
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div className="relative overflow-hidden bg-gradient-to-r from-primary/8 via-card to-secondary/8 border border-border rounded-2xl p-5 sm:p-6 md:p-8">
        ...
        <p className="font-body text-xs sm:text-sm text-muted-foreground mt-2">Manage your students, attendance, and course materials</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5 sm:gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-5 sm:p-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            {quickActions.map((a) => (
              <Link key={a.label} to={a.path} className="flex items-center gap-2.5 p-3 sm:p-4 rounded-xl border border-border hover:bg-primary/5 hover:border-primary/20 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 group-hover:bg-primary/15 group-hover:scale-110 transition-all duration-300">
                  <a.icon className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
                </div>
                <div className="min-w-0">
                  <p className="font-body text-xs sm:text-sm font-semibold text-foreground truncate">{a.label}</p>
                  <p className="font-body text-[10px] sm:text-xs text-muted-foreground truncate">{a.desc}</p>
                </div>
              </Link>
            ))}
          </div>
