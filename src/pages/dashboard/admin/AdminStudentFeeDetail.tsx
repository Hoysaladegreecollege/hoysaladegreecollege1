...
import {
  ArrowLeft, IndianRupee, Calendar, User, BookOpen, Receipt, Phone, Mail, MapPin,
  CheckCircle, AlertCircle, Layers, CreditCard, TrendingUp, Plus, Printer, Bell,
  FileText, Send, Download, MessageCircle, Sparkles, Clock, Eye
} from "lucide-react";
...
export default function AdminStudentFeeDetail() {
  ...
  const recordPayment = useMutation({
    mutationFn: async () => {
      ...
      return { receipt_number, amount, payment_method: payForm.payment_method, remarks, semester: payForm.semester || String(student.semester || 1) };
    },
    onSuccess: (data) => {
      ...
      if (editMode === "semester") {
        ...
        for (const [sem, amount] of entries) {
          await supabase.from("semester_fees").upsert({
            student_id: student.id, semester: parseInt(sem), fee_amount: parseFloat(amount) || 0,
            due_date: editForm.fee_due_date || null, remarks: editForm.fee_remarks || "", updated_by: user?.id,
        ...
        await supabase.from("students").update({ total_fee: totalFromSemesters, fee_due_date: editForm.fee_due_date || null, fee_remarks: editForm.fee_remarks || "" }).eq("id", student.id);
      } else {
        await supabase.from("students").update({ total_fee: parseFloat(editForm.total_fee) || 0, fee_due_date: editForm.fee_due_date || null, fee_remarks: editForm.fee_remarks || "" }).eq("id", student.id);
      }
    },
    onSuccess: () => {
  ...
  const sendReminder = useMutation({
    mutationFn: async () => {
      ...
      try {
        await supabase.functions.invoke("send-push-notification", {
          body: { notifications: [{ user_id: student.user_id, title: "⚠️ Fee Payment Reminder", body: msg, url: "/dashboard/student/fees", tag: `hdc-fee-reminder-${Date.now()}` }] },
        });
      } catch (pushErr) { console.error("Push failed:", pushErr); }
      const studentEmail = student.profile?.email;
      if (studentEmail) {
        try {
    ...
    onSuccess: () => { toast.success("Reminder sent (in-app + push + email)!"); setReminderMsg(""); setShowReminderModal(false); },
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center space-y-3">
          ...
          <Link to="/dashboard/admin/fees" className="inline-flex items-center gap-1.5 text-primary text-sm font-medium hover:underline">
  ...
  payments.forEach((p: any) => { const sem = p.semester || 0; semPayments[sem] = (semPayments[sem] || 0) + Number(p.amount); });
  const semChartData = [1, 2, 3, 4, 5, 6].map(s => ({ name: `Sem ${s}`, paid: semPayments[s] || 0, due: Math.max(0, perSemFee - (semPayments[s] || 0)) }));

  const methodBreakdown: Record<string, number> = {};
  payments.forEach((p: any) => { const m = p.payment_method || "Cash"; methodBreakdown[m] = (methodBreakdown[m] || 0) + Number(p.amount); });
  const methodChartData = Object.entries(methodBreakdown).map(([name, value]) => ({ name, value }));

  // Payment timeline for area chart
  const paymentTimeline = (() => {
    const months: Record<string, number> = {};
    payments.forEach((p: any) => { const m = format(new Date(p.created_at), "MMM yy"); months[m] = (months[m] || 0) + Number(p.amount); });
  ...
  const chartTooltipStyle = {
    contentStyle: { background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "12px", fontSize: "12px", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl">
        ...
        <div className="relative p-6 md:p-8">
          <div className="flex items-start gap-4 flex-wrap">
            <Link to="/dashboard/admin/fees" className="p-2.5 rounded-xl bg-muted/40 hover:bg-muted/60 border border-border/50 transition-all duration-200 shrink-0 mt-1 group">
            ...
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-full px-3 py-1 mb-3">
                ...
                <span className="font-body text-[10px] font-bold uppercase tracking-widest text-primary">Student Fee Report</span>
              </div>
              <h2 className="font-display text-2xl font-bold text-foreground tracking-tight">{student.profile?.full_name || "—"}</h2>
              <p className="font-body text-sm text-muted-foreground mt-1">{student.roll_number} · {student.courses?.name || "—"} ({student.courses?.code || ""}) · Semester {currentSemester}</p>
            </div>
            <div className={`px-4 py-2 rounded-2xl text-xs font-bold border ${isOverdue ? "bg-destructive/10 text-destructive border-destructive/20" : totalDue > 0 ? "bg-amber-500/10 text-amber-500 border-amber-500/20" : "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"}`}>
          ...
          <div className="relative flex flex-wrap gap-2 mt-6 pt-5 border-t border-border/40">
            <Button size="sm" onClick={() => { setShowPayModal(true); setPayForm(f => ({ ...f, semester: String(currentSemester) })); }} className="gap-1.5 text-xs rounded-xl bg-gradient-to-r from-primary to-primary/90 hover:shadow-[0_4px_20px_hsl(var(--primary)/0.3)] transition-all duration-300">
            ...
            <Button size="sm" variant="outline" onClick={exportStudentCSV} className="gap-1.5 text-xs rounded-xl border-border/60 hover:bg-muted/50">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="flex items-start gap-3 p-3 rounded-xl bg-muted/20 border border-border/30 hover:bg-muted/30 transition-colors duration-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "TOTAL FEE", value: `₹${totalFee.toLocaleString()}`, icon: IndianRupee, gradient: "from-primary/15 to-primary/5", iconBg: "bg-primary/15", iconColor: "text-primary", valueColor: "text-foreground" },
          { label: "COLLECTED", value: `₹${feePaid.toLocaleString()}`, icon: CheckCircle, gradient: "from-emerald-500/15 to-emerald-500/5", iconBg: "bg-emerald-500/15", iconColor: "text-emerald-500", valueColor: "text-emerald-500" },
          { label: "PENDING", value: `₹${totalDue.toLocaleString()}`, icon: AlertCircle, gradient: totalDue > 0 ? "from-destructive/15 to-destructive/5" : "from-emerald-500/15 to-emerald-500/5", iconBg: totalDue > 0 ? "bg-destructive/15" : "bg-emerald-500/15", iconColor: totalDue > 0 ? "text-destructive" : "text-emerald-500", valueColor: totalDue > 0 ? "text-destructive" : "text-emerald-500" },
          { label: "COLLECTION", value: `${pct}%`, icon: TrendingUp, gradient: pct >= 80 ? "from-emerald-500/15 to-emerald-500/5" : pct >= 50 ? "from-amber-500/15 to-amber-500/5" : "from-destructive/15 to-destructive/5", iconBg: pct >= 80 ? "bg-emerald-500/15" : pct >= 50 ? "bg-amber-500/15" : "bg-destructive/15", iconColor: pct >= 80 ? "text-emerald-500" : pct >= 50 ? "text-amber-500" : "text-destructive", valueColor: pct >= 80 ? "text-emerald-500" : pct >= 50 ? "text-amber-500" : "text-destructive" },
        ].map(({ label, value, icon: Icon, gradient, iconBg, iconColor, valueColor }) => (
          <div key={label} className="group relative overflow-hidden rounded-2xl border border-border/50 bg-card/80 backdrop-blur-xl p-5 hover:border-border transition-all duration-300 hover:shadow-lg">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl p-6">
        ...
        <div className="relative h-5 bg-muted/50 rounded-full overflow-hidden border border-border/30">
          ...
          <div className="h-full rounded-full transition-all duration-1000 relative overflow-hidden" style={{
            width: `${pct}%`,
            background: pct === 100 ? "linear-gradient(90deg, hsl(142 70% 40%), hsl(142 70% 50%))" : pct > 50 ? "linear-gradient(90deg, hsl(42 87% 45%), hsl(42 87% 55%))" : "linear-gradient(90deg, hsl(0 84% 50%), hsl(0 84% 60%))"
          }}>
        ...
        {student.fee_due_date && (
          <div className={`mt-4 flex items-center gap-2 text-xs font-body px-3 py-2 rounded-xl ${isOverdue ? "bg-destructive/10 text-destructive border border-destructive/20" : "bg-muted/30 text-muted-foreground border border-border/30"}`}>
        ...
        {student.fee_remarks && (
          <p className="font-body text-xs text-muted-foreground mt-3 italic bg-muted/20 rounded-xl px-3 py-2 border border-border/20">💬 {student.fee_remarks}</p>
        )}
      </div>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl p-6">
        ...
        <h3 className="font-display text-sm font-bold text-foreground mb-5 flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center"><Layers className="w-3.5 h-3.5 text-primary" /></div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          {[1, 2, 3, 4, 5, 6].map(s => {
            ...
            return (
              <div key={s} className={`relative overflow-hidden rounded-2xl border p-4 transition-all duration-300 hover:shadow-md ${isCurrent ? "border-primary/40 bg-primary/5" : "border-border/40 bg-muted/10 hover:border-border/60"}`}>
                {isCurrent && <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary to-primary/50" />}
                <div className="flex items-center justify-between mb-2">
                  <span className="font-body text-xs font-bold text-foreground">Sem {s}</span>
                  {isCurrent && <span className="text-[8px] bg-primary text-primary-foreground px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider">Now</span>}
                </div>
                <p className="font-body text-xs text-emerald-500 font-semibold">₹{paid.toLocaleString()} <span className="text-muted-foreground font-normal text-[10px]">paid</span></p>
                <p className="font-body text-xs text-destructive font-semibold">₹{due.toLocaleString()} <span className="text-muted-foreground font-normal text-[10px]">due</span></p>
                <div className="h-1.5 bg-muted/50 rounded-full overflow-hidden mt-3 border border-border/20">
                  <div className="h-full rounded-full transition-all duration-700" style={{
                    width: `${semPct}%`,
                    background: semPct === 100 ? "hsl(142 70% 45%)" : semPct > 50 ? "hsl(42 87% 50%)" : "hsl(0 84% 55%)"
                  }} />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {methodChartData.length > 0 && (
          <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl p-6">
            <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-border/40 to-transparent" />
            <h3 className="font-display text-sm font-bold text-foreground mb-4 flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center"><CreditCard className="w-3.5 h-3.5 text-primary" /></div>
              Payment Methods
            </h3>
            <div className="flex items-center gap-6 flex-wrap">
              <div className="h-44 w-44">
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie data={methodChartData} cx="50%" cy="50%" innerRadius={35} outerRadius={65} dataKey="value" paddingAngle={4} stroke="none">
              ...
              <div className="space-y-2.5 flex-1">
                {methodChartData.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-3 p-2 rounded-xl hover:bg-muted/20 transition-colors">
                    <div className="w-3 h-3 rounded-full shadow-sm" style={{ background: CHART_COLORS[i % CHART_COLORS.length] }} />
                    <span className="font-body text-xs text-foreground font-medium flex-1">{d.name}</span>
                    <span className="font-body text-xs text-muted-foreground tabular-nums">₹{d.value.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {paymentTimeline.length > 1 && (
          <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl p-6">
            <div className="absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-border/40 to-transparent" />
            <h3 className="font-display text-sm font-bold text-foreground mb-4 flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center"><Clock className="w-3.5 h-3.5 text-primary" /></div>
              Payment Timeline
            </h3>
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={paymentTimeline}>
                  ...
                  <Tooltip formatter={(v: number) => `₹${v.toLocaleString()}`} {...chartTooltipStyle} />
                  <Area type="monotone" dataKey="amount" stroke="hsl(142, 70%, 45%)" fill="url(#studentAreaGrad)" strokeWidth={2} name="Amount" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/80 backdrop-blur-xl">
        ...
        <div className="p-6 border-b border-border/40">
          <h3 className="font-display text-sm font-bold text-foreground flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center"><Receipt className="w-3.5 h-3.5 text-primary" /></div>
        ...
        {payments.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="font-body text-xs text-muted-foreground/60 mt-1">Click "Record Payment" to add the first transaction.</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[640px]">
              <thead>
                <tr className="border-b border-border/40">
                  <th className="text-left font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Receipt</th>
                  <th className="text-left font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Date</th>
                  <th className="text-left font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Semester</th>
                  <th className="text-left font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Method</th>
                  <th className="text-right font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Amount</th>
                  <th className="text-left font-body text-[10px] font-bold text-muted-foreground uppercase tracking-wider p-4">Remarks</th>
                </tr>
              </thead>
              <tbody>
                {payments.map((p: any, i: number) => (
                  <tr key={p.id} className="border-b border-border/20 hover:bg-muted/10 transition-colors duration-200 group">
                    <td className="p-4"><span className="font-body text-xs font-mono font-bold text-primary">{p.receipt_number || "—"}</span></td>
                    <td className="p-4">
                    ...
                    <td className="p-4"><span className="font-body text-xs bg-muted/40 border border-border/30 px-2 py-1 rounded-lg">{p.semester ? `Sem ${p.semester}` : "—"}</span></td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right"><span className="font-display text-sm font-bold text-emerald-500 tabular-nums">₹{Number(p.amount).toLocaleString()}</span></td>
              ...
              <tfoot>
                <tr className="border-t-2 border-border/60">
                  <td colSpan={4} className="p-4 font-display text-sm font-bold text-foreground">Total</td>
                  <td className="p-4 text-right font-display text-base font-bold text-emerald-500 tabular-nums">₹{payments.reduce((s: number, p: any) => s + Number(p.amount), 0).toLocaleString()}</td>
                  <td className="p-4" />
                </tr>
              </tfoot>
            </table>
      ...
      <Dialog open={showReceipt} onOpenChange={setShowReceipt}>
        <DialogContent className="sm:max-w-md rounded-3xl border-border/60 bg-card/95 backdrop-blur-2xl shadow-[0_20px_80px_-20px_hsl(var(--primary)/0.15)]">
          ...
          <DialogHeader>
            <DialogTitle className="font-display text-lg font-bold flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-emerald-500/10 flex items-center justify-center"><Receipt className="w-4 h-4 text-emerald-500" /></div>
              Payment Receipt
            </DialogTitle>
          </DialogHeader>
          {lastReceipt && (
            <div className="space-y-4 mt-2">
              <div className="text-center py-5 rounded-2xl bg-gradient-to-b from-emerald-500/10 to-emerald-500/5 border border-emerald-500/20">
                ...
                <p className="font-display text-3xl font-bold text-emerald-500 tabular-nums">₹{Number(lastReceipt.amount).toLocaleString()}</p>
          ...
          <div className="space-y-4 mt-2">
            ...
            {editMode === "total" ? (
              ...
              <div className="space-y-3">
                <p className="font-body text-xs text-muted-foreground">Set fee for each semester. Total fee auto-calculates.</p>
                <div className="grid grid-cols-2 gap-3">
                  {[1, 2, 3, 4, 5, 6].map(sem => (
                    <div key={sem}>
                      ...
                      <div className="relative">
                        ...
                        <input type="number" value={semesterFees[sem] || ""} onChange={e => setSemesterFees(prev => ({ ...prev, [sem]: e.target.value }))} className={`${inputClass} pl-7`} placeholder="0" />
                      </div>
                    </div>
                  ))}
                </div>
                <div className="bg-primary/5 border border-primary/15 rounded-xl p-3 flex items-center justify-between">
                  ...
                  <span className="font-display text-lg font-bold text-primary tabular-nums">₹{Object.values(semesterFees).reduce((sum, v) => sum + (parseFloat(v) || 0), 0).toLocaleString()}</span>
                </div>
              </div>
            )}
            <div>
      ...
      <Dialog open={showReminderModal} onOpenChange={setShowReminderModal}>
        <DialogContent className="sm:max-w-md rounded-3xl border-border/60 bg-card/95 backdrop-blur-2xl shadow-[0_20px_80px_-20px_hsl(var(--primary)/0.15)]">
          ...
          <DialogHeader>
            <DialogTitle className="font-display text-lg font-bold flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-amber-500/10 flex items-center justify-center"><Bell className="w-4 h-4 text-amber-500" /></div>
              Send Fee Reminder
            </DialogTitle>
          ...
          <div className="space-y-4 mt-2">
            <div className="bg-muted/20 border border-border/30 rounded-xl p-4">
              <p className="font-body text-xs text-muted-foreground mb-3">This will send an in-app + push + email notification to the student.</p>
              <div className="flex items-center gap-4">
                <div className="text-center">
                  <p className="font-display text-lg font-bold text-destructive tabular-nums">₹{totalDue.toLocaleString()}</p>
                  <p className="font-body text-[10px] text-muted-foreground uppercase tracking-wider">Pending</p>
                </div>
                {student.fee_due_date && (
                  <div className="text-center">
                    <p className={`font-display text-lg font-bold ${isOverdue ? "text-destructive" : "text-foreground"}`}>{format(new Date(student.fee_due_date), "dd MMM")}</p>
