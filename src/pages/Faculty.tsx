...
const staticFaculty = [
  {
    ...
    qualification: "M.Sc, M.Ed, TET, KSET, Ph.D",
    experience: "20+ years",
    photo_url: "",
    email: "",
    phone: "",
  ...
  return (
    <div className="page-enter">
      <SEOHead
        ...
        description="Meet the experienced faculty of Hoysala Degree College – qualified professors across Computer Applications, Commerce, and Business Administration departments."
        canonical="/faculty"
      />
        ...
        <div className="container px-4 relative">
          <ScrollReveal>
            <SectionHeading
              ...
              subtitle="Experienced professors and industry experts shaping the future"
            />
          </ScrollReveal>
          ...
          {isLoading ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6 max-w-6xl mx-auto">
              {faculty.map((f: any, i: number) => {
                const cfg = deptConfig[f.department] || {
                  grad: "from-primary/10 to-secondary/5",
                ...
                return (
                  <ScrollReveal key={f.id} delay={i * 60}>
                    ...
                    >
                      {/* Hover gradient */}
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${cfg.grad} opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl`}
                      />
                      {/* Top accent line */}
                      ...
                      {/* Photo */}
                      <div
                        className={`relative w-24 h-24 mx-auto rounded-2xl bg-gradient-to-br ${cfg.grad} flex items-center justify-center mb-4 group-hover:scale-110 group-hover:rotate-2 transition-all duration-500 overflow-hidden border-2 border-border group-hover:border-primary/25 shadow-md z-10`}
                      >
                        {f.photo_url ? (
                          <img src={f.photo_url} alt={f.name} className="w-full h-full object-cover" />
                        ) : (
                          <GraduationCap
                            className={`w-10 h-10 ${cfg.iconColor} group-hover:scale-110 transition-transform duration-300`}
                          />
                        )}
                        {/* Shimmer */}
                      ...
                      <h3 className="font-display text-base font-bold text-foreground group-hover:text-primary transition-colors duration-300 relative z-10">
                        {f.name}
                      </h3>
                      <p className="font-body text-xs text-secondary font-semibold mt-1 relative z-10">{f.role}</p>
                      <span
                        className={`inline-block mt-2 font-body text-[10px] font-semibold px-3 py-1 rounded-full border ${cfg.badge} relative z-10`}
                      >
                        {f.department}
                      </span>
                      ...
                      {(f.email || f.phone) && (
                        <div
                          className="flex items-center justify-center gap-2 mt-3 pt-3 border-t border-border/40 relative z-10"
                          onClick={(e) => e.stopPropagation()}
                        >
                          {f.email && (
                            <a
                              href={`mailto:${f.email}`}
                              className="p-2 rounded-xl hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all duration-200 hover:scale-110"
                            >
                          ...
                          {f.phone && (
                            <a
                              href={`tel:${f.phone}`}
                              className="p-2 rounded-xl hover:bg-emerald-500/10 text-muted-foreground hover:text-emerald-600 transition-all duration-200 hover:scale-110"
                            >
      ...
      {selectedFaculty && (
        <div
          className="fixed inset-0 bg-black/70 dark:bg-black/80 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-4 pb-8 sm:pt-24 overflow-y-auto animate-fade-in"
          onClick={() => setSelectedFaculty(null)}
        >
          <div className="my-auto mt-20 sm:mt-12">
            <div
              className="bg-card rounded-3xl border border-border w-full max-w-md shadow-2xl animate-scale-bounce overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header gradient */}
              <div
                className={`p-6 bg-gradient-to-br ${deptConfig[selectedFaculty.department]?.grad || "from-primary/10 to-secondary/5"} relative overflow-hidden`}
              >
                ...
                <div className="flex items-start gap-4 relative z-10">
                  <div className="w-16 h-16 rounded-2xl bg-background/80 flex items-center justify-center shrink-0 overflow-hidden border-2 border-background shadow-md">
                    {selectedFaculty.photo_url ? (
                      ...
                      <GraduationCap
                        className={`w-8 h-8 ${deptConfig[selectedFaculty.department]?.iconColor || "text-primary"}`}
                      />
                    )}
                  </div>
                  <div className="flex-1">
                    ...
                    <span
                      className={`inline-block mt-1.5 font-body text-xs font-semibold px-2.5 py-0.5 rounded-full border ${deptConfig[selectedFaculty.department]?.badge || "bg-muted text-muted-foreground border-border"}`}
                    >
                  ...
                  <button
                    onClick={() => setSelectedFaculty(null)}
                    className="p-1.5 rounded-xl hover:bg-muted transition-colors text-muted-foreground text-sm font-bold hover:scale-110 transition-all duration-200"
                  >
                    ✕
                  </button>
                </div>
                  ...
                  <div className="p-3 rounded-xl bg-muted/50 border border-border/40">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {selectedFaculty.subjects.map((s: string) => (
                        <span
                          key={s}
                          className="font-body text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/15"
                        >
                          {s}
                        </span>
                      ))}
                  ...
                  <div className="flex gap-2 pt-1">
                    {selectedFaculty.email && (
                      <a
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-border hover:bg-primary/5 hover:border-primary/30 font-body text-xs font-semibold transition-all duration-200 group"
                      >
                    ...
                    {selectedFaculty.phone && (
                      <a
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-border hover:bg-emerald-500/5 hover:border-emerald-300 font-body text-xs font-semibold transition-all duration-200 group"
                      >
