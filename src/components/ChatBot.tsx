...
import { X, Send, ArrowUp, Bot, ChevronDown } from "lucide-react";
...
export default function ChatBot() {
  ...
  return (
    <>
      ...
      {/* Chatbot launcher button - Clean minimal with ripple */}
      {!isOpen && (
        <button
          onClick={handleOpen}
          className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center group ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          aria-label="Open chat assistant"
        >
          {/* Ripple rings */}
          <span className="absolute inset-0 rounded-full border-2 border-primary/40 animate-[ripple_2s_ease-out_infinite]" />
          <span className="absolute inset-0 rounded-full border-2 border-primary/20 animate-[ripple_2s_ease-out_0.6s_infinite]" />
          <span className="absolute inset-0 rounded-full border border-primary/10 animate-[ripple_2s_ease-out_1.2s_infinite]" />
          {/* Icon */}
      ...
      {isOpen && (
        <div
          className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[400px] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-in border border-border/50 bg-card ${isMinimized ? "h-auto" : "h-[72vh] sm:h-[560px] max-h-[calc(100vh-5rem)]"}`}
        >
          {/* Header */}
          <div className="relative bg-gradient-to-r from-primary to-primary/85 text-primary-foreground px-4 py-3.5 flex items-center justify-between shrink-0 overflow-hidden">
            ...
            <div className="relative flex items-center gap-1">
              <button onClick={() => setIsMinimized(!isMinimized)} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors" aria-label="Minimize">
                <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isMinimized ? "rotate-180" : ""}`} />
              </button>
              <button onClick={() => setIsOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors" aria-label="Close">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            <>
              ...
              <div className="p-3 border-t border-border flex items-center gap-2 shrink-0 bg-card">
                ...
                <button onClick={() => handleSend(input)} disabled={isLoading || !input.trim()}
                  className="w-10 h-10 rounded-xl bg-primary text-primary-foreground flex items-center justify-center hover:bg-primary/90 transition-all duration-200 shrink-0 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-105 active:scale-95">
                  {isLoading ? (
