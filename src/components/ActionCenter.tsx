...
import { ArrowRight, Bell, Clock, FileText, Mail, Megaphone } from "lucide-react";
...
export default function ActionCenter({ role }: { role: "admin" | "principal" | "teacher" | "student" }) {
  ...
  return (
    <div className="bg-card border border-border/60 rounded-2xl p-5 sm:p-6">
      <h3 className="font-body text-[13px] font-semibold text-foreground mb-4">Action Center</h3>
        ...
        {sorted.map((action, i) => (
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
              action.priority === "high" ? "bg-destructive/10" : action.priority === "medium" ? "bg-secondary/10" : "bg-primary/8"
            }`}>
            ...
            <div className="flex-1 min-w-0">
              <p className="font-body text-[13px] font-medium text-foreground">{action.label}</p>
              <p className="font-body text-[11px] text-muted-foreground">{action.desc}</p>
            </div>
            {action.count !== undefined && action.count > 0 && (
              <span className={`font-body text-[11px] font-semibold px-2 py-0.5 rounded-md shrink-0 ${
                action.priority === "high" ? "bg-destructive/10 text-destructive" : "bg-primary/8 text-primary"
              }`}>
                {action.count}
              </span>
            )}
            <ArrowRight className="w-3.5 h-3.5 text-muted-foreground/40 group-hover:text-muted-foreground group-hover:translate-x-0.5 transition-all duration-200 shrink-0" />
          </Link>
        ))}
      </div>
    </div>
